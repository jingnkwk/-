	...
	<view class="page">
		<view class="search-title">
			<view class="search-input">
				<input type="text" value="" placeholder="搜索漫画名或者作者" v-model="mhname" />
				<view class="icon">
	...
	export default {
		...
		methods:{
			...
			onSearch(){
				...
				//存入本地
				 uni.setStorageSync('mhnameData', this.mhname);
				 //取出本地缓存数据
				 this.getStor();
				 
				 uni.navigateTo({
				 	url:"../search_content/search_content?mhname="+this.mhname
				 })
			...
			goSearch(title){
				...
				uni.navigateTo({
					url:"../search_content/search_content?mhname="+title
				})
			},
			...
			godetails(){
				uni.navigateTo({
					url:"../search_content/search_content?mhname="+this.mhnames
				})
			},
			//取出本地缓存数据
			
			getStor(){
				 let mhnames = uni.getStorageSync('mhnameData');
			...
			clearStor(){
				uni.showModal({
				    ...
				    success:  (res)=> {
				        if (res.confirm) {
				             uni.removeStorageSync('mhnameData');
							 //取出本地缓存数据
							 this.getStor();
				        } else if (res.cancel) {
				            console.log('用户点击取消');
